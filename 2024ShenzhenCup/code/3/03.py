...
for Time_Num in range(len(DATA_DF['time'].iloc[0])):
    ...
    def objective_function(particles):
        ...
        for NUMBER, _particle in enumerate(particles):
            ...
            bounds = [(-180*97.304, 180*97.304), (-90*111.263, 90*111.263), (0, 10000), (13, 23)] #(0, np.min(times_received)
    ...
    # 执行优化
    cost, pos = optimizer.optimize(objective_function, iters=42)

    print(f"第{Time_Num + 1}个残骸的时间组合为：{[Time_Num] + pos.astype(int).tolist()}, 平均误差：{cost/7}")
