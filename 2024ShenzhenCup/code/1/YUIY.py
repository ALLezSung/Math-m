...
# 导入初始数据
DATA_LIST = np.array([
    [110.241, 27.204, 824, 129.03],
    [110.780, 27.456, 727, 105.47],
    [110.712, 27.785, 742, 108.60],
    [110.251, 27.825, 850, 100.61],
    [110.524, 27.617, 786, 32.10],
    [110.467, 27.921, 678, 118.15],
    [110.047, 27.121, 575, 181.33]
])
...
def calculate_position(df, tol=1e-9):
    ...
    def objective_function(variables, points, times_received):
            ...
            error.append(np.mean(np.abs(euclid_distances - sound_distances)))
    ...
    result = minimize(objective_function, x0, 
                      ...
                      method='L-BFGS-B', options={'ftol': tol, 'disp': False},
    ...
    if result.success:
        ...
        return RESULT_POINT,error  # 返回信号源位置
    else:
...
if __name__ == '__main__':
    ...
    # 调用函数进行计算
    calculated_position,__ = calculate_position(DATA_DF)
    print(f'监测设备信息:\n{DATA_DF}\n预测目标点信息:\n{calculated_position}')
    ...
    chart1 = fg.add_subplot(211)
    chart1.plot(np.arange(len(__)), __)
    chart1.set_xlabel('Iteration')
    chart1.set_ylabel('Error')
