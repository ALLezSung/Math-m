...
def move_files(raw_path, op_path, train_scale, valid_scale):
    # 定义图片和标签的文件夹路径
    image_folder = os.path.join(raw_path, 'images')
    label_folder = os.path.join(raw_path, 'labels')
    ...
    for phase in [('train', train_images), ('valid', valid_images), ('test', test_images)]:
        ...
        for image_name in phase_images:
            src_image_path = os.path.join(image_folder, image_name + '.jpg')
            dst_image_path = os.path.join(op_path, phase_name, 'images', image_name + '.jpg')
...
if __name__ == "__main__":
    raw_path = "E:\\.app\\GitsDepository\\Gitshub-projects\\datasets\\my_dss"
    op_path = "E:\\.app\\GitsDepository\\Gitshub-projects\\datasets\\my_dss_split"
    train_scale = 0.8
    valid_scale = 0.2
    test_scale = 0.0

    move_files(raw_path, op_path, train_scale, valid_scale)
    print("Done.")